...
ranges.each_pair do |office, netblock|
  ...
  netblock.each do |block|
    ...
    if block.kind_of?(String) # ["1.1.1.0/24"]
    ...
    ip_range.each do |ip|
      ...
      langs.each do |lang|
        ...
        # STDERR.puts url
        edits = JSON.parse(open(url).read)
        contribs = edits["query"]["usercontribs"]
        if contribs.length > 0
          contribs.each do |contrib|
            puts [ ip, lang, contrib['title'], contrib['timestamp'], contrib['pageid'], contrib['revid'], contrib['parentid'], contrib['sizediff']].to_csv
            STDERR.print "*" # If we found something.  Exciting!
          end
          more_to_get = false
          if edits["query-continue"]
            # There are more to get, because we hit the maximum number of results in one query.  Loop through, using the
            # timestamp given, until there's nothing left.
            more_to_get = true
            while more_to_get
              ...
              edits = JSON.parse(open(url_for_more).read)
              contribs = edits["query"]["usercontribs"]
